...
interface Analyzer {
  methodId: string;
  name: string;
  analyze: (priceData: PriceData, db?: any) => Promise<AnalysisResult>;
}
 ...
 */
export function createAnalyzer(methodConfig: MethodConfig): Analyzer {
  return {
    ...
    analyze: async (priceData: PriceData, db: any = null): Promise<AnalysisResult> => {
      ...
      try {
        ...
        try {
          const { validateAIResponse } = await import('../services/groq-client');
...
export async function buildUserPrompt(priceData: PriceData, db: any, methodId: string): Promise<string> {
  ...
  if (db) {
    try {
      ...
      const formatPredictionHistory = (history: any[], coinName: string): string => {
        ...
        filtered.forEach(h => {
          ...
          predictions.forEach((p: any) => {
            ...
            if (p.timeframe === '4h' || p.timeframe === '1d') {
              const hoursAgo = Math.round((new Date().getTime() - new Date(h.timestamp).getTime()) / (1000 * 60 * 60));
    ...
    console.log(`[AnalyzerFactory][${methodId}] Historical context disabled, skipping positions/orders context`);
  }

  // Calculate actual changes for each timeframe to help Groq
  const calcChange = (arr: number[] | undefined): number => {
  ...
  // Fetch OHLC candle data for Kim Nghia method (BTC only, 24 candles for rate limit)
  let ohlcContext = '';
 ...
 */
async function formatAnalysisResponse(rawResponse: any, priceData: PriceData, methodId: string, db: any): Promise<AnalysisResult> {
  ...
  let kimNghiaFibonacci: any = null;
  if (methodId === 'kim_nghia' && db) {
    try {
  ...
  const validatePositionDecisions = (decisions: any[], bias: string, openPositions: any[]): any[] | null => {
  ...
  const validatePendingOrderDecisions = (decisions: any[]): any[] | null => {
    if (!decisions || !Array.isArray(decisions)) return null;
  ...
  const formatCoin = (coinData: any, currentPrice: number, coin: string): any => {
    // Validate and fix prediction targets
    const validatePredictionTarget = (pred: any, currentPrice: number): any => {
    ...
    const validatePriceLevel = (price: any, currentPrice: number, type: string, bias: string | null = null, suggestedEntry: number | null = null): number | null => {
      ...
      if (type === 'stop_loss' && suggestedEntry) {
        ...
        const minSlDistancePercent = getSymbolPolicy(coin).minSlDistancePercent || 0.005;
