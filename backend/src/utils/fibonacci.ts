...
interface FibonacciResultWithSwing extends FibonacciResult {
  swingHigh: number | null;
  swingLow: number | null;
}
 ...
 */
export function detectSwingPoints(ohlcData: OHLCData[], lookback: number = 20): SwingPoints {
 ...
 */
export function calculateFibonacciLevels(swingHigh: number, swingLow: number, direction: string = 'up'): FibonacciResult {
 ...
 */
export function getFibonacciFromOHLC(ohlcData: OHLCData[], direction: string = 'up', lookback: number = 20): FibonacciResultWithSwing {
  const { swingHigh, swingLow } = detectSwingPoints(ohlcData, lookback);
  const levels = calculateFibonacciLevels(swingHigh || 0, swingLow || 0, direction);
