...
Hệ thống tích hợp Binance Futures Testnet cho phép thực hiện giao dịch thực tế trên môi trường testnet song song với paper trading system. Điều này giúp so sánh hiệu quả giữa 2 hệ thống trước khi triển khai lên mainnet.
...
1. **Binance Client Service** (`backend/src/services/binanceClient.js`)
   - Wrapper around `@binance/connector` library
...
- [@binance/connector NPM Package](https://www.npmjs.com/package/@binance/connector)
